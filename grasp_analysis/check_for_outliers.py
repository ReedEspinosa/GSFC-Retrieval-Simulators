...
def extract_aod_data(matched_pairs, aod_var_name="aod"):
    ...
    Safely broadcasts lat/lon to match multi-dimensional AOD fields.
    """
    monthly_data = {}
    high_aod_records = []
...
def plot_aod_difference(monthly_data, out_filename="aod_difference_vs_truth.png"):
    """
    Creates a scatter plot showing (Retrieved - Truth) difference vs Truth AOD.
    """
    all_truth = []
    all_retrieved = []
    for month in monthly_data:
    ...
    difference = all_retrieved - all_truth
    ...
    plt.title(f"AOD Difference (Retrieved - Truth) vs Truth AOD (N={len(all_truth)})")
    plt.xlabel("Truth AOD")
    plt.ylabel("Difference (Retrieved AOD - Truth AOD)")
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.legend()
    
    plt.tight_layout()
    plt.savefig(out_filename, dpi=300)
    print(f"Saved AOD difference plot to {out_filename}")
    plt.close()
...
if __name__ == "__main__":
    ...
    if matched_pairs:
        ...
        plot_aod_difference(monthly_data)  # <--- New plot added here
