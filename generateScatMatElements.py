...
elmnts = ['p11', 'p12', 'p33', 'p34', 'p22', 'p44']
...
for i in range(Nplts):
    ...
    if i==0:
        plt.plot(rslt[0]['angle'],rslt[0][elmnts[0]])
        plt.yscale('log')
        plt.ylabel('$S_{11}$')
    else:
        plt.plot(rslt[0]['angle'],rslt[0][elmnts[i]]/rslt[0][elmnts[0]])
...
plt.suptitle('Sulfate(RH=70%) [binSet02]: GRASP (solid) vs Optics Tables (dashed)')
