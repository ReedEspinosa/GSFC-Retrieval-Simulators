...
def readPetesAngleFiles(filePtrn, nAng=10, verbose=False, saveData=True, loadData=True):
    ...
    filePtrn - Full path with wildcard (*) from which glob can grab all relevant files
        e.g. filePtrn = '/Users/wrespino/Synced/Remote_Sensing_Projects/A-CCP/angularSampling/colarco_20200511_pdfs/ss450/ss450.random.2006*.txt'
    ...
    if loadData == True:
        try:
            ...
            if verbose: print('Could not load valid pickle data from %s. Processing text files...' % pklPath)    
    fileNames = np.sort(glob(filePtrn))
